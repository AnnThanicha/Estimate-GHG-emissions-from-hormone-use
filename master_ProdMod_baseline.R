...
# Object initialisation ---------------------------------------------------
  ...
  cow[["status"]]         <- rep(1, n_cows) # categorical variable (1 = healthy; 2 = culled for fertility reasons; 3 = culled at the end of useful life(max par);
                                            # 4 = culled for general reasons; 5 = mortality; 6 = annoestrus cows; 0 = no cow in space)
  ...
  cow[["daycalf"]]        <- (cow[["mdl"]] + DPL) - (cow[["dim"]]) # day of calving (initialisation)
  cow[["prevdaycalf"]]    <- rep(0, n_cows)
  cow[["pregnant"]]       <- rep(1, n_cows) # ifelse(cow[["daycalf"]] > cow[["dim"]], 1, 0)        # variable indicating pregnancy status 
  cow[["daypregnant"]]    <- round(rnorm(length(n_cows), 281, 3)) - (cow[["daycalf"]]) #start the day of pregnancy for initialisation
  cow[["pregloss"]]       <- rep(0, n_cows)    # pregnancy loss event
  ...
  cow[["n_insem_pregloss"]] <- rep(0, n_cows)  #count number of AI if pregnancy loss occurs
  cow[["insem_day"]]      <- rep(0, n_cows)    # records day of last insemination
  ...
  cow[["cost.cull.mort"]] <- rep(0, n_cows)
  cow[["cost.milk"]]      <- rep(0, n_cows)
  cow[["cost.milk.disc"]] <- rep(0, n_cows)
  cow[["cost.PD"]]        <- rep(0, n_cows)     # cost of pregnancy diagnosis
  cow[["cost.hormone"]]   <- rep(0, n_cows)     # hormone costs CEVA
  cow[["cost.hormone.an"]]   <- rep(0, n_cows)  # hormone costs baseline PRIDsynch
  cow[["cost.hormone.cod"]]   <- rep(0, n_cows) # hormone costs baseline OvSynch
  cow[["cost.hormone.hi"]]    <- rep(0, n_cows) # hormone costs baseline PG
  cow[["cost.labour.ai"]]    <- rep(0, n_cows)
  cow[["cost.labour.hu"]]    <- rep(0, n_cows)
  cow[["cost.calf"]]      <- rep(0, n_cows)
  cow[["rev.milk"]]       <- rep(0, n_cows)
  cow[["rev.calf"]]       <- rep(0, n_cows)
  
  #CEVA
  cow[["PTCL_status"]]   <- rep(0, n_cows) # protocol status (0 = no protocol; 1 = double ovsynch; 2 = ovsynch; 3 = PRID synch; 4 = open cows to be detected for heat (CevaScen 2:3))
  cow[["pregCheck_sdl"]] <- rep(0, n_cows) # pregnancy check scheduling variable  
  
  cow[["an_cyc1"]] <- rep(0, n_cows)
  cow[["cod"]]     <- rep(0, n_cows)
  cow[["suboest"]] <- rep(0, n_cows)
  cow[["treated"]] <- rep(0, n_cows)
  cow[["treat_day"]] <- rep(0, n_cows)
  ...
  cow.entries   <- n_cows
  day           <- 0
  glb_PD        <- 7  # global pregnancy diagnosis scheduling variable
...
# General culling ---------------------------------------------------------
    ...
    # find cows that are not pregnant after n days after first oestrus and/or failed to conceive after max inseminations
      ...
      fertcull1 <- which(cow[["pregnant"]] == 0 & cow[["calf"]] == 0 & cow[["n_insem"]] >= max_insem)
      fertcull2 <- which(cow[["pregnant"]] == 0 & cow[["calf"]] == 0 & cow[["n_insem_pregloss"]] >= max_insem_pregloss)
      fertcull3 <- which(cow[["pregnant"]] == 0 & cow[["calf"]] == 0 & cow[["dim"]] > 20 & cow[["yield"]] < yield_thresh.AI & cow[["status"]] == 1) # DIM > 20 is to prevent the culling rule for the early lactation cow after calving, status = 1 to prevent gen cull cow being fert cull
      fertcull <- c(fertcull1, fertcull2, fertcull3)
...
# Data collection ---------------------------------------------------------
  ...
  # out = list(cowsList = cowsList, check = oestCheck)
  # return(out)
  cowsList
} # :: end of model ::
...
MySim2<- function(nFrames = xPRM$n_sim, limS = NA, clb = NA) {
  ...
  t. <- format(Sys.time(), "%F %H-%M") ; t. <- gsub(" ", "_", t.)
  save(xPRM, file = paste0("default_Parameters_v02_", t., ".RData"))
  for(i in seq.int(1,nFrames,1)) {
    if(xPRM$n_sim > limS){
      frame <- rbindlist(mkFrames(X = xPRM))
      frame$.id <- i
      frame <- frame[frame$year > 6, ]  #USE THIS ONLY to start from year 7 because the burn in is on year 7 (to save some space), remove this if you want to have the data from beginning!
